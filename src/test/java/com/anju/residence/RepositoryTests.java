...
//    log.info(sceneRepo.findFirstByUserAndIdName(u.getId(), name).toString());
    ...
    void script() {
        ...
        List<DeviceVO> devices = deviceRepo.findAllVoByUserId(user.getId());
        ...
        for (DeviceVO device : devices) {
            ...
            for (int i = 0; i < num; i++) {
                ...
                ElectricLog electricLog = ElectricLog.builder().power(power).consumption(power * 5).time(DateUtil.getDateMinusMinute(i * 5)).device(Device.builder().id(device.getId()).build()).build();
    ...
    @Test
    void deviceRepoTest() {
        Pageable page = PageRequest.of(10000, 10, Sort.unsorted());
        log.info("GOGO");
        DeviceVO deviceVo = deviceRepo.findVoById(18000).orElse(new DeviceVO());
//    List<DeviceVO> deviceVos = deviceRepo.findVoByStatus(0, page);
        log.info(deviceVo.toString());
        deviceVo = deviceRepo.findVoById(180).orElse(new DeviceVO());
        log.info(deviceVo.toString());
//    log.info(String.valueOf(deviceVos.size()));
//    for (DeviceVO dv : deviceVos) {
//      log.info(dv.toString());
//    }
    }

    @Test
    void WaterMeterRepoTest() {
        List<WaterMeterVO> waterMeterVoS = waterMeterRepo.findAllByUserId(405);
        for (WaterMeterVO vo : waterMeterVoS) {
